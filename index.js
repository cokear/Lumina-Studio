...
// Silent mode: redirect all console output to void
...
async function generateConfig() {
  const config = {
    ...
    inbounds: [
      { port: ARGO_PORT, protocol: 'vless', settings: { clients: [{ id: UUID }], decryption: 'none', fallbacks: [{ dest: 3001 }, { path: "/vless-argo", dest: 3002 }, { path: "/vmess-argo", dest: 3003 }, { path: "/trojan-argo", dest: 3004 }] }, streamSettings: { network: 'tcp' }, sniffing: { enabled: true, destOverride: ["http", "tls"] } },
...
async function downloadFilesAndRun() {
  ...
  if (KOMARI_ENDPOINT && KOMARI_TOKEN) {
    ...
    filesToAuthorize = [npmPath, webPath, botPath];
  } else {
    filesToAuthorize = [phpPath, webPath, botPath];
  }
